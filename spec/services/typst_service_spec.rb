...
describe TypstService do
  ...
  describe 'compilation root' do
    ...
    it 'refuses to read outside of it', :external_deps do
      ...
      # (a scratch compilation root holds the rogue template: specs never
      # write into the repository)
      Dir.mktmpdir('typst-root') do |dir|
        File.write(File.join(dir, 'escape.typ'), '#let render(data) = read("/../config/database.yml")')
        stub_const('TypstService::ROOT', Pathname(dir))

        expect { described_class.generate_pdf('escape', {}) }
  ...
  describe '.with_assets' do
    ...
    it 'embeds the image with its alt text in a PDF/UA-1 document', :external_deps do
      ...
      # (a scratch compilation root mirroring lib/typst/root through links
      # holds the ad-hoc template: specs never write into the repository)
      Dir.mktmpdir('typst-root') do |dir|
        root = Pathname(dir)
        TypstService::ROOT.children.each { File.symlink(it, root.join(it.basename)) }
        stub_const('TypstService::ROOT', root)
        stub_const('TypstService::IMAGES_DIR', root.join('images'))
        root.join('illustration.typ').write(<<~TYPST)
          #import "/theme.typ": *
          #let render(data) = {
        ...
        described_class.with_assets do |assets|
          data = { **Typst::Payload.new.letterhead, map: assets.image(blob, alt: 'Carte de la zone') }
