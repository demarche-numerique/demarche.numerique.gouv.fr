...
describe TypeDeChampTree do
  ...
  # the stable ids in document order
  def flat_stable_ids(nodes) = nodes.flat_map { [it.stable_id, *flat_stable_ids(it.children)] }
  ...
  describe '.from_coordinates' do
    ...
    context 'whatever the header sections' do
      ...
      it 'preserves the document order' do
        expect(flat_stable_ids(tree.public_children)).to eq(revision.public_flat_type_de_champs.map(&:stable_id))
        expect(flat_stable_ids(tree.private_children)).to eq(revision.private_flat_type_de_champs.map(&:stable_id))
      end
    end
