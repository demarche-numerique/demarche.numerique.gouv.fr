...
describe Typst::DossierVidePayload do
  it 'is named after its template' do
    expect(described_class.template).to eq('dossier_vide')
    expect(TypstService::ROOT.join('dossier_vide.typ')).to be_file
  end
  ...
  describe 'header and identity' do
    context 'procedure for a legal entity' do
      ...
      it 'carries the title, logo and establishment identity' do
        ...
        expect(TypstService::ROOT.join(payload[:logo][:path].delete_prefix("/"))).to be_file
  ...
  describe 'dispatch per champ type' do
    ...
    context 'list too large to print' do
      ...
      before { stub_const('Typst::DossierVidePayload::MAX_PRINTABLE_OPTIONS', 3) }
