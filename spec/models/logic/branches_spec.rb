...
describe Logic::Branches do
  ...
  context 'with an optional source' do
    ...
    it 'has a blank branch where nothing is displayed' do
      ...
      expect(clusters.first.branches.map { it.regions[champ_value(2)] }).to include(Logic::PossibleValues::Blank)
      expect(clusters.first.branches).to all(satisfy { |branch| adult?(branch.regions[champ_value(2)]) == branch.visible.include?(5) })
    end

    def adult?(region) = region != Logic::PossibleValues::Blank && !region.restrict(Logic::GreaterThanEq, 18).empty?
  end

  context 'with an optional checkbox' do
    let(:checkbox) { build(:type_de_champ_checkbox, stable_id: 1, mandatory: false) }
    let(:type_de_champs) { [checkbox, if_yes, if_no] }

    it 'is checked or not, never blank' do
      expect(clusters.first.branches.map { it.regions[champ_value(1)] }).not_to include(Logic::PossibleValues::Blank)
      expect(visible_sets(clusters.first)).to contain_exactly([3], [4])
    end
  end
  ...
  context 'with a condition still being written' do
    let(:choice) { build(:type_de_champ_drop_down_list, stable_id: 12, mandatory: true, drop_down_options: []) }
    let(:dependent) { build(:type_de_champ_text, stable_id: 13, condition: ds_eq(champ_value(12), empty)) }
  ...
  context 'with a conditional source' do
    ...
    it 'hides what depends on a hidden source, comparison by comparison' do
  ...
  context 'with as many branches as the cap can hold' do
    ...
    it 'decides each comparison once per region rather than once per branch' do
      # per source: its values, its two regions, and each region narrowed once by the comparison
      expect(Logic::PossibleValues::Enum).to receive(:new).exactly(13 * 5).times.and_call_original
      expect(clusters.first.branches.size).to eq(2**13)
    end
  end
  ...
  context 'with a multiple choice mentioning more options than the cap can hold' do
    ...
    it 'leaves the cluster unenumerated without building the regions' do
      allow(Logic::PossibleValues::Enums).to receive(:new).and_call_original
      expect(Logic::PossibleValues::Enums).not_to receive(:new).with(hash_including(:must_include))
      expect(clusters.first).to be_capped
    end
  end
