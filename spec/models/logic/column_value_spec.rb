...
describe Logic::ColumnValue do
  ...
  describe '#remap_procedure_id' do
    it 'returns a ColumnValue whose to_h carries the new procedure_id' do
      ...
      expect(remapped.to_h.dig("column_id", :procedure_id)).to eq(42)
      expect(remapped.to_h.dig("column_id", :column_id)).to eq(column.h_id[:column_id])
    end

    it 'leaves the original ColumnValue untouched' do
      column_value.remap_procedure_id(42)

      expect(column_value.to_h.dig("column_id", :procedure_id)).to eq(procedure.id)
    end

    it 'remaps a broken ColumnValue without raising' do
      broken_h_id = { procedure_id: 999_999, column_id: "type_de_champ/123" }
      broken = Logic::ColumnValue.from_h("term" => "Logic::ColumnValue", "column_id" => broken_h_id)

      remapped = broken.remap_procedure_id(7)

      expect(remapped.to_h.dig("column_id", :procedure_id)).to eq(7)
      expect(remapped.to_h.dig("column_id", :column_id)).to eq("type_de_champ/123")
    end

    it 'remaps recursively through binary operators' do
      condition = Logic::Eq.new(column_value, Logic::Constant.new(true))

      remapped = condition.remap_procedure_id(42)

      expect(remapped).to be_a(Logic::Eq)
      expect(remapped.left.to_h.dig("column_id", :procedure_id)).to eq(42)
      expect(remapped.right).to eq(Logic::Constant.new(true))
    end

    it 'remaps recursively through n-ary operators' do
      condition = Logic::And.new([
        Logic::Eq.new(column_value, Logic::Constant.new(true)),
        Logic::Eq.new(Logic::ChampValue.new(column.stable_id), Logic::Constant.new(false)),
      ])

      remapped = condition.remap_procedure_id(42)

      expect(remapped).to be_a(Logic::And)
      expect(remapped.operands.first.left.to_h.dig("column_id", :procedure_id)).to eq(42)
      # ChampValue is unaffected
      expect(remapped.operands.last.left).to eq(Logic::ChampValue.new(column.stable_id))
    end
  end
end
