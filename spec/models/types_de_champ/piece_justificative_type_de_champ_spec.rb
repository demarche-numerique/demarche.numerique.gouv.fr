...
describe TypesDeChamp::PieceJustificativeTypeDeChamp do
  describe '#columns' do
    ...
    it 'adds RIB columns' do
      ...
      cols = TypesDeChamp::TypeDeChampBase.build(tdc).columns(procedure_id: procedure.id, displayable: true)
  ...
  describe '#champ_value_for_export' do
    context 'when nature is titre_identite' do
      ...
      it 'returns nil when no file attached (blank champs export the default)' do
        expect(type_de_champ.champ_value_for_export(champ)).to be_nil
      end
      ...
      it 'returns "présent" when file attached' do
        ...
        expect(type_de_champ.champ_value_for_export(champ)).to eq('présent')
      end
    end
    ...
    context 'when nature is not titre_identite' do
      ...
      it 'returns filenames' do
        champ.piece_justificative_file.attach(fixture_file_upload('spec/fixtures/files/logo_test_procedure.png', 'image/png'))
        expect(type_de_champ.champ_value_for_export(champ)).to include('logo_test_procedure.png')
      end

      it 'returns nil when no file (blank champs export the default)' do
        expect(type_de_champ.champ_value_for_export(champ)).to be_nil
      end
    end
  end
  ...
  describe '#champ_value_for_api' do
    ...
    it 'returns url for first file in v1 when safe' do
      ...
      expect(champ.type_de_champ.champ_value_for_api(champ, version: 1)).to include('/rails/active_storage/')
    end
    ...
    it 'returns nil when infected' do
      ...
      expect(champ.type_de_champ.champ_value_for_api(champ, version: 1)).to be_nil
    end
  end
end
