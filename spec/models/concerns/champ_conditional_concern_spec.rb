...
describe ChampConditionalConcern do
  ...
  let(:champ) do
    dossier.project_champs_public.find { _1.stable_id == 99 }.tap { _1.update_column(:value, '1.1234') }
  end
  let(:last_champ) do
    dossier.project_champs_public.find { _1.stable_id == 999 }.tap { _1.update_column(:value, '1.1234') }
  end
  let(:condition) { nil }
  ...
  describe '#visible?' do
    ...
    context 'inside a repetition' do
      ...
      let(:first_repet) { dossier.project_champs_public.find(&:repetition?) }
      let(:first_yes_no) { first_repet.rows.first.find(&:yes_no?) }
  ...
  describe '#submitted_filled?' do
    ...
    context 'when dossier not on submitted revision' do
      ...
      context 'when champ is empty' do
        before { champ.writable!.update(value: nil) }
        it { expect(champ.submitted_filled?).to be_falsey }
      end
    end
  end
