...
describe DossierSectionsConcern do
  describe '#auto_numbering_section_headers_for?' do
    ...
    let(:public_type_de_champ) { dossier.root_public_types_de_champ[1] }
    let(:private_type_de_champ) { dossier.root_private_types_de_champ[1] }
    ...
    context "header_section in a repetition are not auto-numbered" do
      ...
      let(:public_type_de_champ) { dossier.root_public_types_de_champ[1].flat_children.first }
  ...
  describe '#index_for_section_header' do
    ...
    let(:headers) { dossier.root_public_types_de_champ.filter(&:header_section?) }
