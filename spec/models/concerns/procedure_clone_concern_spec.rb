...
describe ProcedureCloneConcern, type: :model do
  describe 'clone' do
    ...
    let(:type_de_champ_repetition) { procedure.draft_revision.root_public_types_de_champ.last }
    let(:type_de_champ_private_repetition) { procedure.draft_revision.root_private_types_de_champ.last }
    ...
    it 'should duplicate specific objects with different id' do
      ...
      expect(subject.draft_revision.root_public_types_de_champ.size).to eq(procedure.draft_revision.root_public_types_de_champ.size)
      expect(subject.draft_revision.root_private_types_de_champ.size).to eq(procedure.draft_revision.root_private_types_de_champ.size)

      procedure.draft_revision.root_public_types_de_champ.zip(subject.draft_revision.root_public_types_de_champ).each do |ptc, stc|
        expect(stc).to have_same_attributes_as(ptc)
        expect(stc.revisions).to include(subject.draft_revision)
      end

      public_repetition = type_de_champ_repetition
      cloned_public_repetition = subject.draft_revision.root_public_types_de_champ.find(&:repetition?)
      public_repetition.flat_children.zip(cloned_public_repetition.flat_children).each do |ptc, stc|
        expect(stc).to have_same_attributes_as(ptc)
        expect(stc.revisions).to include(subject.draft_revision)
      end

      procedure.draft_revision.root_private_types_de_champ.zip(subject.draft_revision.root_private_types_de_champ).each do |ptc, stc|
        expect(stc).to have_same_attributes_as(ptc)
        expect(stc.revisions).to include(subject.draft_revision)
      end

      private_repetition = type_de_champ_private_repetition
      cloned_private_repetition = subject.draft_revision.root_private_types_de_champ.find(&:repetition?)
      private_repetition.flat_children.zip(cloned_private_repetition.flat_children).each do |ptc, stc|
        expect(stc).to have_same_attributes_as(ptc)
        expect(stc.revisions).to include(subject.draft_revision)
      end
    ...
    context 'when types_de_champ_public contains a referentiel' do
      ...
      context 'when cloned by the same administrateur' do
        ...
        it 'keeps API keys' do
          expect(subject.draft_revision.root_public_types_de_champ.first.referentiel.authentication_method).to eq(referentiel.authentication_method)
          expect(subject.draft_revision.root_public_types_de_champ.first.referentiel.authentication_data).to eq(referentiel.authentication_data)
        end
      end
      ...
      context 'when cloned by another administrateur' do
        ...
        it 'discards API keys' do
          expect(subject.draft_revision.root_public_types_de_champ.first.referentiel.authentication_data).to eq(nil)
        end
      end
    end
    ...
    context 'when the procedure is cloned from the library' do
      ...
      it 'should discard old pj information' do
        subject.draft_revision.root_public_types_de_champ.each do |stc|
    ...
    it 'should keep types_de_champ ids stable' do
      expect(subject.draft_revision.root_public_types_de_champ.first.id).not_to eq(procedure.draft_revision.root_public_types_de_champ.first.id)
      expect(subject.draft_revision.root_public_types_de_champ.first.stable_id).to eq(procedure.draft_revision.root_public_types_de_champ.first.id)
    end

    it 'should duplicate piece_justificative_template on a type_de_champ' do
      expect(subject.draft_revision.root_public_types_de_champ.find(&:piece_justificative?).piece_justificative_template.attached?).to be_truthy
    end
    ...
    context 'with a drop_down_list referentiel' do
      ...
      let(:drop_down_list) { procedure.draft_revision.root_public_types_de_champ.first }
      let(:cloned_drop_down_list) { subject.draft_revision.root_public_types_de_champ.first }
