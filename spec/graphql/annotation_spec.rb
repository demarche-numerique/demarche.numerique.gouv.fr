...
RSpec.describe Mutations::DossierModifierAnnotations, type: :graphql do
  ...
  let(:private_champs) { dossier.root_champs_private }
  ...
  describe 'dossierModifierAnnotationText' do
    ...
    context 'with rows' do
      let(:annotation) { private_champs.find(&:repetition?).rows.first.first }
      let(:other_annotation) { private_champs.find(&:repetition?).rows.second.first }
  ...
  describe 'dossierModifierAnnotationDecimalNumber' do
    ...
    context 'with rows' do
      let(:annotation) { private_champs.find(&:repetition?).rows.first.find(&:decimal_number?) }
      let(:other_annotation) { private_champs.find(&:repetition?).rows.second.find(&:decimal_number?) }
