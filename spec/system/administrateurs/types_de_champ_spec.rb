...
describe 'As an administrateur I can edit types de champ', js: true do
  ...
  scenario "adding a piece justificative template" do
    ...
    tdc = procedure.active_revision.root_public_types_de_champ.first
  ...
  scenario "adding a repetition champ" do
    ...
    wait_until do
      tdc = procedure.active_revision.reload.root_public_types_de_champ.first
  ...
  context 'header section' do
    scenario 'with public tdc, having invalid order, it pops up errors summary' do
      ...
      wait_until { procedure.reload.active_revision.root_public_types_de_champ.first&.type_champ == TypeDeChamp.type_champs.fetch(:header_section) }
      first_header = procedure.active_revision.root_public_types_de_champ.first
      ...
      wait_until { procedure.reload.active_revision.root_public_types_de_champ.count == 2 }
      second_header = procedure.active_revision.root_public_types_de_champ.last
      select('Titre de section', from: dom_id(second_header, :type_champ))
      wait_until { procedure.reload.active_revision.root_public_types_de_champ.last&.type_champ == TypeDeChamp.type_champs.fetch(:header_section) }
      ...
      procedure.reload.active_revision.root_private_types_de_champ.each do |header_section|
  ...
  context 'move and morph' do
    ...
    let!(:initial_first_coordinate) { procedure.draft_revision.public_revision_types_de_champ[0] }
    let!(:initial_second_coordinate) { procedure.draft_revision.public_revision_types_de_champ[1] }
    let!(:initial_third_coordinate) { procedure.draft_revision.public_revision_types_de_champ[2] }
