...
describe SandboxedVips, ".ensure_usable!", :external_deps, if: SANDBOX_USABLE do
  ...
  # A decoder can be installed, executable, and still never start where it matters — a
  # library outside LIBRARIES, an AppArmor profile naming it. Boot is where that shows.
  it "refuses a decoder the sandbox cannot run" do
    allow(SandboxedCommand).to receive(:run)
      .and_return(["", "error while loading shared libraries: libvips.so.42", instance_double(Process::Status, success?: false)])

    expect { described_class.ensure_usable! }.to raise_error(RuntimeError, /will not run in the sandbox: error while loading/)
  end

  it "reads the version of the libvips it would run, in the sandbox" do
