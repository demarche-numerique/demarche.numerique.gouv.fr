...
module Maintenance
  RSpec.describe T20260918BackfillProcedureRevisionTypeDeChampTreeTask do
    ...
    it "n’enregistre rien sur le brouillon" do
      expect { process }.not_to change { procedure.draft_revision.reload.read_attribute(:type_de_champ_tree) }.from(nil)
    end

    # la démarche est chargée avec son lot, et peut être publiée avant d’être traitée
    it "n’enregistre rien sur le brouillon né d’une publication survenue depuis le chargement de la démarche" do
      ...
      expect(stale_procedure.draft_revision_id).not_to eq(procedure.reload.draft_revision_id)
      expect(procedure.draft_revision.read_attribute(:type_de_champ_tree)).to be_nil
