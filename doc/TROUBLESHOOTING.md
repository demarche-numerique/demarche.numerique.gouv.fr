...
- On an instance running with `BWRAP_ISOLATION`, ensure the `libvips-tools` package is installed: the isolated path shells out to `vips`, `vipsheader` and `vipsthumbnail`.
