...
- **Expert revocation** — two independent revocations end an expert's access, and they compose: `avis.revoked_at` (an instructeur revokes one avis) and `experts_procedures.revoked_at` (an admin revokes the expert from the procedure, effective only while `experts_require_administrateur_invitation`). `ExpertsProcedure.granting_access` holds the link half of the rule — and, through Procedure's `kept` default scope, drops the links of a deleted procedure too — while `Avis.not_revoked` composes it — compose those rather than re-expressing either with a hand-written `where(revoked_at: nil)`. Read an expert's own avis through `expert.avis`, which is already scoped; `dossier.avis` is deliberately unscoped, because the instructeur sees revoked avis as history. Anything sent to an expert — mail, notification, export — must be filtered.
