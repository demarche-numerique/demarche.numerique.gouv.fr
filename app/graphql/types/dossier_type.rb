...
module Types
  class DossierType < Types::BaseObject
    ...
    def messages(id: nil)
      records = dataloader.with(Sources::Association, commentaires_chronological: [:instructeur, :expert]).load(object)
      if id.present?
    ...
    def champs(id: nil)
      if id.present?
        find_record_by_typed_id(object.root_public_champs, id, attribute: :stable_id)
      else
        object.root_public_champs.filter(&:visible?)
      end
    end

    def annotations(id: nil)
      if id.present?
        find_record_by_typed_id(object.root_private_champs, id, attribute: :stable_id)
      else
        object.root_private_champs.filter(&:visible?)
      end
    end
    ...
    def attestation
      if object.termine? && (object.attestation_acceptation_template&.activated? || object.attestation_refus_template&.activated?)
