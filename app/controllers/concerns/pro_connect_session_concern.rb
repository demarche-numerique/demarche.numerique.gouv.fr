...
module ProConnectSessionConcern
  ...
  def administrateur_device?
    cookies[ADMINISTRATEUR_DEVICE_COOKIE_NAME].present?
  end

  def pro_connect_only_login?
    params[:force_pro_connect].present? || (administrateur_device? && params[:particulier].blank?)
  end
