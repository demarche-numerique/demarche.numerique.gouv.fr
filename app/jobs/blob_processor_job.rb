...
class BlobProcessorJob < ApplicationJob
    ...
    Tempfile.create(["processed", File.extname(path)]) do |output|
      if watermark_needed
        decode(path, autorotate: autorotate_needed) do |image|
          WatermarkService.new.apply(image, format: blob.content_type).write_to_file(output.path, **write_opts)
        end
      else
        rewrite(path, output.path, autorotate: autorotate_needed, **write_opts)
      end
  ...
  def rewrite(path, target, autorotate:, **save_opts)
    return SandboxedVips.rewrite(path, target, autorotate:, **save_opts) if SandboxedCommand::ENABLED

    load_opts = { access: :sequential }
    load_opts[:autorotate] = true if autorotate

    Vips::Image.new_from_file(path, **load_opts).write_to_file(target, **save_opts)
  end
