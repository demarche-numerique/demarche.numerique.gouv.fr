...
class SessionFailureApp < Devise::FailureApp
  ...
  # `delete`, not read: the hook fires on opportunistic fetches too, so a later
  # unrelated failure in the same request would otherwise inherit the message.
  def i18n_message(default = nil)
    reason = request.env.delete(SessionRegistrableConcern::END_REASON_KEY)

    return super if warden_options[:recall].present? || reason.blank?
