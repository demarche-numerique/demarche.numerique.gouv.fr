...
module SandboxedVips
  ...
  class << self
    def ensure_usable!
      DECODERS.each { ensure_decoder_runs!(it) }

      raise "libvips #{version} ignores VIPS_BLOCK_UNTRUSTED, the sandbox needs #{MIN_VERSION} or newer" if version < MIN_VERSION
    end

    # Read in the sandbox rather than on the host: it is the one that will decode.
    def version
      output, = run([VIPS, "--version"])

      Gem::Version.new(output[/\d+\.\d+\.\d+/])
    end
    ...
    def ensure_decoder_runs!(decoder)
      raise "#{decoder} is missing — on Debian and Ubuntu it ships in libvips-tools" if !File.exist?(decoder)

      _, error, status = run([decoder, "--version"])
      raise "#{decoder} will not run in the sandbox: #{failure_message(error, status)}" if !status.success?
    end
