  ...
  def dossiers_count_summary(groupe_instructeur_ids)
    query = <<~EOF
      SELECT
        ...
        COUNT(DISTINCT dossiers.id) FILTER (where dossiers.hidden_by_administration_at IS NULL AND dossiers.hidden_by_expired_at IS NULL AND dossiers.state in ('accepte', 'refuse', 'sans_suite') AND (dossiers.expired_at - INTERVAL '#{Expired::REMAINING_WEEKS_BEFORE_EXPIRATION} weeks' < :now)) AS expirant
      FROM dossiers
