...
class ProcedureRevision < ApplicationRecord
  ...
  #
  # A locked revision can no longer change, so it is cached on its own key
  # alone, without loading its champs; a draft is also keyed on its champs
  # (their number and latest change), so that it is recomputed once per edit
  # rather than on every render.
  def estimated_fill_duration_range
    Rails.cache.fetch([self, *draft_champs_version, 'estimated_fill_duration_range'], expires_in: 12.hours) do
  ...
  def draft_champs_version
    return [] if locked?

    tdcs = type_de_champs

    [tdcs.size, tdcs.map(&:updated_at).max&.utc&.to_fs(:usec)]
  end
