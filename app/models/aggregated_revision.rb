...
#
# Built in memory from the revisions' trees; nothing is cached beyond the
# instance.
class AggregatedRevision
  ...
  def tree
    @tree ||= TypeDeChampTree.new(
      public_coordinates: coordinates_for(merged_members[:public]),
      private_coordinates: coordinates_for(merged_members[:private])
    )
  end

  # Newest first: the first revision a stable_id appears in provides its
  # version and its place.
  def revisions
    if @procedure.brouillon?
      [@procedure.draft_revision]
    else
      (@procedure.revisions.to_a - [@procedure.draft_revision]).sort_by(&:created_at).reverse
    end
  end

  # {container => ordered stable_ids of its direct members}, containers being
  # :public, :private or a section/repetition stable_id. Each type de champ is
  # placed exactly once — walking revisions newest first, in the newest
  # container it belonged to, after the members already placed there.
  def merged_members
    @merged_members ||= begin
      ...
      revisions.each do |revision|
        place = lambda do |container, types_de_champ|
          types_de_champ.each do |type_de_champ|
            ...
            @latest_version[stable_id] ||= type_de_champ
  ...
  def coordinates_for(stable_ids)
    stable_ids.flat_map do |stable_id|
      type_de_champ = @latest_version[stable_id]
      if type_de_champ.repetition?
        [Coordinate.new(type_de_champ:, children: coordinates_for(merged_members[stable_id]))]
