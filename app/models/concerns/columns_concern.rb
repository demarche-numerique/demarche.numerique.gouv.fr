...
module ColumnsConcern
  ...
  def form_filterable_columns
    aggregated_root_types_de_champ_public.filter(&:fillable?).flat_map { _1.columns(procedure_id: id) }.filter(&:filterable)
  end

  def annotation_privees_filterable_columns
    aggregated_root_types_de_champ_private.filter(&:fillable?).flat_map { _1.columns(procedure_id: id) }.filter(&:filterable)
  end
  ...
  def dossier_dates_columns
    ['created_at', 'updated_at', 'last_champ_updated_at', 'last_champ_instructeur_updated_at', 'depose_at', 'en_construction_at', 'en_instruction_at', 'processed_at', 'expired_at']
  ...
  def types_de_champ_columns
    (aggregated_root_types_de_champ_public + aggregated_root_types_de_champ_private)
      .filter(&:fillable?)
      .filter(&:dynamic_type)
