...
# Logic::PossibleValues#regions), plus blank when the champ is optional; a branch is a
# choice of one region per source, and lists the champs it displays. A champ
# is displayed when its condition holds, an comparison on a champ that is itself
...
class Logic::Branches
  ...
  def max_regions_of(source, members)
    ...
    count = values ? values.max_regions(comparisons) : 0
  ...
  def regions_of(source, members)
    ...
    regions = values ? values.regions(comparisons) : []
    regions << Logic::PossibleValues::Blank if regions.empty? || !source.mandatory?
  ...
  def values_and_comparisons(source, members)
    ...
    @values_and_comparisons[source.term] ||= [
      ...
      members.flat_map { comparisons_on(it.condition, source.term) },
    ]
  end

  def comparisons_on(condition, term)
    condition.terms
      .filter { it.checkable? && it.left == term }
