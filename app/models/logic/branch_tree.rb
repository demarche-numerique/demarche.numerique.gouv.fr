...
class Logic::BranchTree
  ...
  Node = Data.define(:source, :regions, :children, :visible) do
    ...
    # « Salarié, Retraité » or, when the values are listed as such,
    # « avec A ou avec B, sans A »
    def to_s
      described = regions.map { it.to_s(source.type_de_champ) }

      described.join(described.any? { it.include?(', ') } ? I18n.t('logic.possible_values.or') : ', ')
    end
  end
  ...
  # when the values can tell the union, listed otherwise.
  def merge(nodes)
  ...
  # By the values' own order (numbers ascending, blank last), ties as
  # enumerated.
  def sort(nodes)
